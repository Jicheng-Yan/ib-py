...
class EWrapperMsgGenerator(AnyWrapperMsgGenerator):
    ...
    def tickOptionComputation(cls, tickerId,
                                   ...
                                   theta,
                                   undPrice):
        toAdd = "id=" + tickerId + "  " + TickType.getField(field) + ": vol = " + str(impliedVol) if impliedVol >= 0 and (impliedVol != float('inf')) else "N/A" + " delta = " + str(delta) if abs(delta) <= 1 else "N/A" + " gamma = " + str(gamma) if abs(gamma) <= 1 else "N/A" + " vega = " + str(vega) if abs(vega) <= 1 else "N/A" + " theta = " + str(theta) if abs(theta) <= 1 else "N/A" + " optPrice = " + str(optPrice) if optPrice >= 0 and (optPrice != float('inf')) else "N/A" + " pvDividend = " + str(pvDividend) if pvDividend >= 0 and (pvDividend != float('inf')) else "N/A" + " undPrice = " + str(undPrice) if undPrice >= 0 and (undPrice != float('inf')) else "N/A"
        return toAdd
